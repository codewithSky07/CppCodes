...
public:
    ...
    int latest_non_ov_opt(vector<vector<int>> &v,int i)
    {
        ...
        { 
            int mid = (low + high) / 2; 
            if (v[mid][0] <= v[i][1])  // end time is less then start time of current job.
            { 
                if (v[mid + 1][0] <= v[i][1]) // increment the low. normal binary search.
                    low = mid + 1; 
...
//         To store the profit upto that job.
        ...
        for(int i=1;i<n;i++)
        {
            ...
            int inclusive = v[i][2]; // profit of new job to be included if we do job
            int index = latest_non_ov_opt(v,i);
